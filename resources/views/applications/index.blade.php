<x-app-layout>
    @push('head')
        <style>.applications-page tbody tr { transition: background-color .15s ease; } .applications-page tbody tr:hover { background: #ecfdf5 !important; }</style>
    @endpush
    @push('head')
        {{-- EmailJS SDK for sending approval/rejection emails --}}
        <script src="https://cdn.jsdelivr.net/npm/@emailjs/browser@4/dist/email.min.js"></script>
        <script>
            (function() {
                var key = '{{ env("EMAILJS_PUBLIC_KEY", "") }}';
    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div class="flex items-center">
                <div class="p-2 rounded-full bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-600 mr-3">
            ...
            <a href="{{ route('applications.index') }}" class="inline-flex items-center px-4 py-2.5 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 shadow-sm hover:shadow transition-all duration-200">
    ...
    <div class="applications-page py-2">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <!-- Filter Section -->
            <div class="mb-6 bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-2xl border border-gray-200 dark:border-gray-700">
                <div class="p-4 text-gray-900 dark:text-gray-100">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div class="relative flex-1">
                            ...
                            <input type="text" id="searchInput" placeholder="Search applications..." class="block w-full pl-10 pr-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg leading-5 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-emerald-600 focus:border-emerald-600 sm:text-sm transition duration-150 ease-in-out">
                        </div>
                        <div class="flex items-center space-x-2">
                            <select id="statusFilter" class="block w-full pl-3 pr-10 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-emerald-600 focus:border-emerald-600 sm:text-sm">
                                <option value="">All Statuses</option>
                                <option value="pending">Pending</option>
                                <option value="approved">Approved</option>
                                <option value="rejected">Rejected</option>
                            </select>
                            <select id="planFilter" class="block w-full pl-3 pr-10 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-emerald-600 focus:border-emerald-600 sm:text-sm">
                                    ...
                                    @foreach($applications as $application)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    <div class="flex-shrink-0 h-10 w-10 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center text-emerald-600 dark:text-emerald-600">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="flex items-center space-x-2">
                                                    @if($application->status === 'pending')
                                                        <button type="button" onclick="confirmApprove('{{ $application->id }}', '{{ $application->company_name }}')" class="inline-flex items-center px-3 py-1.5 bg-emerald-600 text-white rounded-lg text-xs hover:bg-emerald-700 shadow-sm transition-all">
                                                            <i class="fas fa-check mr-1.5"></i>
                                                            Approve
                                                        </button>
                                                        <button type="button" onclick="confirmReject('{{ $application->id }}', '{{ $application->company_name }}')" class="inline-flex items-center px-3 py-1.5 bg-red-600 text-white rounded-lg text-xs hover:bg-red-700 shadow-sm transition-all">
    ...
    <!-- Approve Modal -->
    <div id="approveModal" class="fixed inset-0 bg-black/50 hidden flex items-center justify-center z-50">
        <div x-data="{ show: false }" x-init="setTimeout(() => { show = true }, 100)"
            ...
            <p class="mb-6 text-gray-700 dark:text-gray-300">Are you sure you want to approve the application for <span id="approveApplicationName" class="font-semibold text-emerald-600 dark:text-emerald-600"></span>? This will create a new tenant with their own database and domain.</p>
            <form id="approveForm" method="POST" action="">
                ...
                <div class="flex justify-end space-x-3">
                    ...
                    <button type="submit" class="px-4 py-2.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 shadow-sm transition-all">
    ...
    <div id="rejectModal" class="fixed inset-0 bg-black/50 hidden flex items-center justify-center z-50">
        <div x-data="{ show: false }" x-init="setTimeout(() => { show = true }, 100)"
            ...
            <p class="mb-4 text-gray-700 dark:text-gray-300">Are you sure you want to reject the application for <span id="rejectApplicationName" class="font-semibold text-emerald-600 dark:text-emerald-600"></span>?</p>
            <form id="rejectForm" method="POST" action="">
                @csrf
                <div class="mb-4">
                    <label for="notes" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Reason for rejection</label>
                    <textarea id="notes" name="notes" rows="3" class="w-full px-4 py-3 border border-gray-200 dark:border-gray-700 rounded-lg focus:outline-none focus:ring-emerald-600 focus:border-emerald-600 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100" required placeholder="Please provide a reason for rejecting this application..."></textarea>
                </div>
                <div class="flex justify-end space-x-3">
                    ...
                    <button type="submit" class="px-4 py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700 shadow-sm transition-all">
    ...
    @if(session('emailjs_approved'))
    @php $ejsData = session('emailjs_approved'); @endphp
    <script>
    document.addEventListener('DOMContentLoaded', function () {
        var serviceId  = '{{ env("EMAILJS_SERVICE_ID", "") }}';
        var templateId = '{{ env("EMAILJS_TEMPLATE_APPROVED", "") }}';
        if (!serviceId || !templateId) {
            console.warn('EmailJS approval template not configured in .env');
            return;
        }
        emailjs.send(serviceId, templateId, {
            to_email : '{{ $ejsData["to_email"] }}',
            to_name  : '{{ addslashes($ejsData["to_name"]) }}',
            company  : '{{ addslashes($ejsData["company"]) }}',
            domain   : '{{ $ejsData["domain"] }}',
            password : '{{ $ejsData["password"] }}',
            plan     : '{{ $ejsData["plan"] }}',
        }).then(function(res) {
            console.log('Approval email sent via EmailJS:', res.status);
        }).catch(function(err) {
            console.warn('EmailJS approval email failed (non-blocking):', err);
        });
    });
    </script>
    @endif
