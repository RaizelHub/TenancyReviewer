...
<div class="space-y-6">
    ...
    <div>
        ...
        <textarea id="description" name="description" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-emerald-500 dark:focus:border-emerald-600 focus:ring-emerald-500 dark:focus:ring-emerald-600 rounded-md shadow-sm" rows="4" required>{{ old('description', $activity?->description) }}</textarea>
    ...
    <div>
        ...
        <select id="subject_id" name="subject_id" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-emerald-500 dark:focus:border-emerald-600 focus:ring-emerald-500 dark:focus:ring-emerald-600 rounded-md shadow-sm" required>
    ...
    <div id="activityDocumentField" class="p-4 rounded-lg border">
        ...
        <input type="file" id="activity_document" name="activity_document" class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100" accept=".pdf,.doc,.docx" />
        ...
        @if($activity?->activity_document_path)
            <div class="mt-2">
                <a href="{{ Storage::url($activity->activity_document_path) }}" target="_blank" class="text-sm text-emerald-600 dark:text-emerald-400 hover:underline">
    ...
    <div id="reviewerAttachmentField" class="p-4 rounded-lg border">
        ...
        <input type="file" id="reviewer_attachment" name="reviewer_attachment" class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100" accept=".pdf" />
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Maximum file size: 10MB
            <span class="material-note hidden text-green-600 dark:text-green-400 font-medium ml-2">
        ...
        @if($activity?->reviewer_attachment_path)
            <div class="mt-2">
                <a href="{{ Storage::url($activity->reviewer_attachment_path) }}" target="_blank" class="text-sm text-emerald-600 dark:text-emerald-400 hover:underline">
