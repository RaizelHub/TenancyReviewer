                    ...
                    @else
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach($activitiesWithQuizzes as $activity)
                                <div class="group bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden border border-gray-100 dark:border-gray-700 hover:shadow-lg transition-all duration-300 hover:border-emerald-200 dark:hover:border-emerald-900 hover-lift">
                                    <div class="h-2 bg-emerald-600"></div>
                                    <div class="p-6">
                                        <div class="flex justify-between items-start mb-3">
                                            <div>
                                                <h3 class="text-lg font-semibold text-gray-900 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">{{ $activity->quiz->title }}</h3>
                                        ...
                                        <div class="grid grid-cols-2 gap-3 mb-4">
                                            <div class="bg-emerald-50 dark:bg-emerald-900/10 rounded-lg p-3 text-center">
                                        ...
                                        <div class="mt-4">
                                            @if(isset($attempts[$activity->quiz->id]) && $attempts[$activity->quiz->id]->count() > 0)
                                                ...
                                                <div class="flex space-x-2">
                                                    <a href="{{ route('quizzes.results', $latestAttempt->id) }}" class="flex-1 inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-center text-white bg-gradient-to-r from-blue-600 to-emerald-600 rounded-lg hover:from-blue-700 hover:to-emerald-700 transition-all duration-200 shadow-sm">
                                                        <i class="fas fa-chart-bar mr-1.5"></i>
                                                        View Results
                                                    </a>

                                                    <a href="{{ route('quizzes.start', $activity->quiz->id) }}" class="flex-1 inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-center text-white bg-gradient-to-r from-emerald-600 to-emerald-600 rounded-lg hover:from-emerald-700 hover:to-emerald-700 transition-all duration-200 shadow-sm">
                                                        <i class="fas fa-redo mr-1.5"></i>
                                                        Retake Quiz
                                                    </a>
                                                </div>
                                            @else
                                                <a href="{{ route('quizzes.start', $activity->quiz->id) }}" class="w-full inline-flex items-center justify-center px-4 py-2.5 text-sm font-medium text-center text-white bg-gradient-to-r from-emerald-600 to-emerald-600 rounded-lg hover:from-emerald-700 hover:to-emerald-700 transition-all duration-200 shadow-sm">
