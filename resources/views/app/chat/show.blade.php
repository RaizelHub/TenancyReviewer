<x-tenant-app-layout>
    <div class="py-6">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <!-- Chat container with fixed header -->
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col h-[calc(100vh-10rem)]">
                <!-- Chat header -->
                <div class="p-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex flex-col sticky top-0 z-10">
                    <!-- Subject context banner (if available) -->
                    @php
                        $channelData = [];
                        ...
                        $hasSubjectContext = !empty($channelData['subject_id']);
                        $subjectContext = session('subject_context') ?? null;

                        if (!$hasSubjectContext && $subjectContext) {
                            $hasSubjectContext = true;
                            $channelData['subject_id'] = $subjectContext['id'] ?? null;
                            $channelData['subject_name'] = $subjectContext['name'] ?? null;
                            $channelData['subject_code'] = $subjectContext['code'] ?? null;
                        }
                    @endphp
                    ...
                    <div class="flex items-center">
                        <a href="{{ route('chat.index') }}" class="mr-3 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400">
                                @if(isset($channel['last_message']))
                                    Last active {{ \Carbon\Carbon::parse($channel['last_message']['created_at'])->diffForHumans() }}
                                @else
                                    No activity yet
                                @endif
                            </p>
                        ...
                        <div class="flex items-center space-x-2">
                            <button type="button" class="p-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors" title="View participants">
                                <i class="fas fa-users"></i>
                            </button>
                            <button type="button" class="p-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors" title="More options">
                    ...
                    @if(empty($messages))
                        <div class="text-center py-12 h-full flex flex-col items-center justify-center">
                            ...
                            <p class="text-gray-500 dark:text-gray-400 max-w-sm text-center">
                                Start the conversation by sending a message below. Your messages will appear here.
                            </p>
                        </div>
                    @else
                        <div class="space-y-6 py-2">
                            ...
                            @foreach($messages as $message)
                                @php
                                    ...
                                    $isCurrentUser = $message['user']['user_id'] == $user->id;
                                @endphp

                                @if($showDateDivider)
                                    <div class="flex items-center justify-center my-6">
                                        <div class="bg-gray-200 dark:bg-gray-700 px-3 py-1 rounded-full text-xs text-gray-600 dark:text-gray-300">
                                ...
                                <div class="flex {{ $isCurrentUser ? 'justify-end' : 'justify-start' }} group" data-message-id="{{ $message['message_id'] ?? '' }}">
                                    <div class="flex max-w-[75%] {{ $isCurrentUser ? 'flex-row-reverse' : 'flex-row' }} items-end">
                                        ...
                                        <div class="space-y-1 max-w-full">
                                            <div class="flex items-center {{ $isCurrentUser ? 'justify-end' : 'justify-start' }} text-xs text-gray-500 dark:text-gray-400 mb-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <div id="typing-indicator" class="flex justify-start hidden">
                                <div class="flex max-w-[75%] flex-row items-end">
                                    <div class="flex-shrink-0 h-8 w-8 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-gray-500 dark:text-gray-400 mr-2 mb-1">
                                        <i class="fas fa-user"></i>
                                    </div>

                                    <div class="rounded-2xl px-4 py-2 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-bl-none border border-gray-200 dark:border-gray-600">
                                        <div class="flex space-x-1">
                                            <div class="w-2 h-2 rounded-full bg-gray-400 dark:bg-gray-500 animate-bounce" style="animation-delay: 0ms"></div>
                                            <div class="w-2 h-2 rounded-full bg-gray-400 dark:bg-gray-500 animate-bounce" style="animation-delay: 150ms"></div>
                                            <div class="w-2 h-2 rounded-full bg-gray-400 dark:bg-gray-500 animate-bounce" style="animation-delay: 300ms"></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <form id="message-form" class="flex flex-col space-y-2" enctype="multipart/form-data">
                        ...
                        <div class="flex items-end">
                            <div class="flex-1 relative">
                                ...
                                <button type="button" id="attach-button" class="absolute right-2 bottom-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 p-1">
                        ...
                        <div id="emoji-picker" class="hidden absolute bottom-20 left-4 z-10 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg shadow-lg p-2">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            sb.connect(USER_ID, function(user, error) {
                ...
                sb.GroupChannel.getChannel(CHANNEL_URL, function(channel, error) {
                    ...
                    messageListParams.includeMetaArray = true; // Include meta data
                    messageListParams.includeReactions = true; // Include reactions

                    console.log('Fetching messages with params:', messageListParams);

                    // First try to get messages from the channel directly
                    channel.getMessagesByTimestamp(Date.now(), messageListParams, function(messages, error) {
                        if (error) {
                            console.error('Error loading previous messages from Sendbird SDK:', error);

                            // If SDK method fails, try to use the messages from the server-side
                            const serverMessages = {!! json_encode($messages) !!};
                            console.log('Using server-side messages:', serverMessages);

                            if (serverMessages && serverMessages.length > 0) {
                                displayMessages(serverMessages);
                            }
                        } else {
                            console.log('Previous messages loaded from Sendbird SDK:', messages);

                            if (messages && messages.length > 0) {
                                displayMessages(messages);
                            } else {
                                // If no messages from SDK, try server-side messages
                                const serverMessages = {!! json_encode($messages) !!};
                                console.log('No SDK messages, using server-side messages:', serverMessages);
                    ...
                    function displayMessages(messages) {
                        // Clear existing messages if any were loaded
                        const chatMessages = document.getElementById('chat-messages');
                        ...
                        messages.forEach(function(message) {
                            ...
                            const senderId = message.sender ? message.sender.userId : (message.user ? message.user.user_id : null);
                    ...
                    function sendMessage(event) {
                        ...
                        try {
                            ...
                            if (hasFile) {
                                ...
                                // Send file to server
                                fetch('{{ route('chat.send', $channel["channel_url"]) }}', {
                                    method: 'POST',
                                    headers: {
                    ...
                    function sendMessageToServer(messageText, callback) {
                        return fetch('{{ route('chat.send', $channel["channel_url"]) }}', {
                            method: 'POST',
                            headers: {
                    ...
                    channelHandler.onTypingStatusUpdated = function(channel) {
                        console.log('Typing status updated:', channel);

                        // Get typing users excluding current user
                        const typingUsers = channel.getTypingUsers().filter(user => user.userId !== USER_ID);

                        // Show typing indicator if someone is typing
                        showTypingIndicator(typingUsers.length > 0);

                        // Update typing indicator with user info if needed
                        if (typingUsers.length > 0) {
                            const typingUser = typingUsers[0];
                            const typingAvatar = document.querySelector('#typing-indicator .rounded-full');
                            const typingInitial = typingUser.nickname ? typingUser.nickname.charAt(0).toUpperCase() : 'U';

                            if (typingAvatar) {
                                if (typingUser.profileUrl) {
                                    typingAvatar.innerHTML = `<img src="${typingUser.profileUrl}" alt="${typingUser.nickname}" class="h-8 w-8 rounded-full">`;
                                } else {
                                    typingAvatar.innerHTML = typingInitial;
                                }
                            }
                        }
                    };
